...
export default function FeaturesPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#0A1020] text-white">
      <Navigation />
      <main className="max-w-6xl mx-auto px-2 sm:px-4 mt-24 pb-16">
        {/* Section 1: Hero Banner */}
        <section className="py-12 sm:py-20 flex flex-col items-center text-center bg-[#0A1020] gap-4">
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold mb-4 sm:mb-6 leading-tight">Your AI Growth Toolkit: From Student to Founder.</h1>
          <p className="text-base sm:text-lg md:text-2xl text-[#cbd5e1] mb-6 sm:mb-10 max-w-2xl mx-auto">Our users save 10+ hours every week and achieve 2x faster growth with AI-powered templates, tools, and weekly drops.</p>
          <div className="w-full max-w-xl mx-auto mb-8 sm:mb-10 bg-[#151a28] rounded-2xl p-4 sm:p-6 min-h-[180px]">
            <Line data={hoursSavedData} options={{
              plugins: { legend: { display: false } },
              scales: { x: { grid: { color: '#232946' } }, y: { grid: { color: '#232946' } } },
              elements: { line: { borderWidth: 4 } },
              responsive: true,
              maintainAspectRatio: false,
            }} height={220} />
          </div>
          <Link href="/signup" className="inline-block bg-[#2563eb] hover:bg-[#1d4ed8] text-white font-bold text-base sm:text-xl md:text-2xl px-6 sm:px-10 py-4 sm:py-5 rounded-2xl shadow-lg transition-all duration-150">Start Your Hustle Now.</Link>
        </section>
