...
function getCorrectImportPath(filePath) {
  ...
  if (appIndex === -1) {
    console.log(`❌ Could not find 'app' directory in path: ${filePath}`);
    return null;
  }
...
// Function to find and fix all ComingSoon import paths
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          if (content.includes('import ComingSoon from') && content.includes('components/ComingSoon')) {
